...
class Main {
            ...
            if (this.soundSetCache.get(this.soundSetName)) {
                ...
                recursive(path.join(__dirname, '../soundset/', this.soundSetName, 'audios'), (_, audioPaths) => {
                    ...
                    for (let audioPath of audioPaths) {
                        ...
                        audios.set(path.basename(audioPath), this.createAudio(audioPath))
                    }
    ...
    activate(state) {
        ...
        this.configThrottlingObserver = atom.config.observe('sound.throttling', (isThrottling) => {
    ...
    deactivate(state) {
        ...
        this.configThrottlingObserver.dispose()
    }
    ...
    play(audioFileName) {
        if (this.isThrottling && this.isPlaying) return
        this.isPlaying = true
